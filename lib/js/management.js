...
jQuery(document).ready(function ($) {
    ...
    // close the flightbox
    Xflightbox = function () {
        $('div#ssfa-flightbox', window.parent.document).fadeOut(250, function () {
            $(this).remove();
        });
        $('div#ssfa-flightbox-shadow', window.parent.document).fadeOut(250, function () {
            $(this).remove();
        });
    }
});
    ...
    if ($('th.ssfa-manager').length) {
                    ...
                    (
                        ...
                        function (response) {
                            if (response.status === 'error' || response.status === 'partial') {
                                ...
                                filertify.set({labels: {ok: fileaway_mgmt.ok_label}});
                                filertify.alert(response.message);
                                $(del).parents('tr').fadeOut(250, function () {
                                    $(this).remove();
                                });
                                ...
                                filertify.set({
                                    labels: {
                                        ok: fileaway_mgmt.confirm_label,
                                        cancel: fileaway_mgmt.cancel_label
                                    }
                                });
        ...
        $('body').on('click', 'a[id^="cancel-ssfa-file-"]', function (ev) {
            ...
            for (var i = 0; i <= customs; i++) {
                ...
                $(cdata).fadeOut(250);
                setTimeout(function () {
                    $(cdata).siblings('span').fadeIn(250);
                }, 250);
            }
        });
            ...
            (
                ...
                function (response) {
                    if (response.status == 'success') {
                        del.parents('tr').eq(0).fadeOut(250, function () {
                            $(this).remove();
                        });
                    } else {
                        filertify.set({labels: {ok: fileaway_mgmt.ok_label}});
        ...
        $('span[id^="ssfa_submit_upload_"]').on('click', function () {
            ...
            if (!settings.fixed && $('input#ssfa-upload-actionpath-' + uid).val() === '') {
                ...
                if (TheFiles[uid] !== undefined && TheFiles[uid].length > 0) {
                    $(this).parents('span.ssfa_fileup_wrapper').eq(0).fadeTo(250, 0, function () {
    ...
    if ($('div.ssfa-fileaway-values[data-editor="true"]').length) {
        $('table.ssfa-values-table > thead > tr > th').contextMenu('ssfa-values-rename-column-context', {
            'Rename Column': {
                click: function (element) {
                    fa_values_renamecol(element.attr('id'));
                },
            ...
            'Insert New Column Before': {
                click: function (element) {
                    fa_values_newcol(element.attr('id'), 'before');
                },
            ...
            'Insert New Column After': {
                click: function (element) {
                    fa_values_newcol(element.attr('id'), 'after');
                },
            ...
            'Delete Column': {
                click: function (element) {
                    fa_values_deletecol(element.attr('id'));
                },
            ...
            'Save Backup': {
                click: function (element) {
                    fa_values_backup(element.attr('id'));
                },
        ...
        fa_row_context_menu_init = function () {
            $('tr.ssfa-values-context td').contextMenu('ssfa-values-context', {
                'Insert New Row': {
                    click: function (element) {
                        fa_values_insert_row(element.attr('id'));
                    },
                ...
                'Delete Row': {
                    click: function (element) {
                        fa_values_delete(element.attr('id'));
                    },
        ...
        $('body').on('blur', 'input[id^="input-ssfa-values-"]', function () {
            ...
            $input.fadeOut(250);
            $value = $(this).val();
            if ($value === $oldvalue) setTimeout(function () {
                $span.fadeIn(250);
            }, 250);
            else {
                var shadow = $('<div id="ssfa-values-shadow" style="display:none;" />');
