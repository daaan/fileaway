...
if ( ! class_exists( 'fileaway_prints' ) ) {
    class fileaway_prints {
        ...
        public function __construct() {
            ...
            $this->file2   = $this->options['custom_stylesheet'] && is_file(
                $this->path . '/' . $this->options['custom_stylesheet']
            ) ?
        ...
        public function register() {
            wp_register_script(
                ...
                fileaway_url . '/lib/js/soundmanager2.js',
                array( 'jquery' ),
                '2.97a.20130101'
            );
            wp_register_script( 'fileaway-alphanum', fileaway_url . '/lib/js/alphanum.js', array( 'jquery' ), '1.0' );
            wp_register_script( 'fileaway-chozed',
                fileaway_url . '/lib/js/chosen/chosen.js',
                array( 'jquery' ),
                '1.1.0' );
            wp_register_script( 'fileaway-footable',
                fileaway_url . '/lib/js/footable.js',
                array( 'jquery' ),
                '2.0.1.2' );
            wp_register_script( 'fileaway-filertify',
                fileaway_url . '/lib/js/filertify.js',
                array( 'jquery' ),
                '0.3.11' );
            wp_register_script(
                'fileaway-contextmenu',
                fileaway_url . '/lib/js/context/contextmenu.js',
                array( 'jquery' ),
                fileaway_version
            );
            wp_register_script(
                'fileaway-management',
                fileaway_url . '/lib/js/management.js',
                array( 'jquery' ),
                fileaway_version
            );
            wp_register_script(
                'fileaway-stats',
                fileaway_url . '/lib/js/stats.js',
                array( 'jquery', 'fileaway-management' ),
                fileaway_version
            );
            wp_register_style( 'fileaway-chozed', fileaway_url . '/lib/js/chosen/chosen.css', array(), '1.1.0' );
            wp_register_style( 'fileaway-icons',
                fileaway_url . '/lib/css/fileaway-icons.css',
                array(),
                fileaway_version );
            wp_register_style(
                'fileaway-styles',
                fileaway_url . '/lib/css/fileaway-styles.css',
                array(),
                fileaway_version
            );
            if ( $this->file1 ) {
                wp_register_style(
                    'fileaway-custom-styles',
                    $this->file1,
                    array( 'fileaway-icons', 'fileaway-styles' ),
                    fileaway_version
                );
            }
            if ( $this->file2 ) {
                wp_register_style(
                    'fileaway-custom-stylesheet',
                    $this->file2,
                    array( 'fileaway-icons', 'fileaway-styles' ),
                    fileaway_version
                );
        ...
        public function styles() {
            if ( $this->options['stylesheet'] == 'header' || $GLOBALS['fileaway_add_styles'] ) {
                ...
                if ( $this->file1 ) {
                    wp_enqueue_style( 'fileaway-custom-styles' );
                }
                if ( $this->file2 ) {
                    wp_enqueue_style( 'fileaway-custom-stylesheet' );
                }
            }
        }
        ...
        public function manager() {
            ...
            $vars = array(
                ...
                'delete_confirm'       => sprintf(
                    __( 'You are about to permanently delete %s. Are you sure you\'re OK with that?', 'file-away' ),
                    'numfiles'
                ),
                'tamper1'              => __( 'Sorry, there was a problem verifying the correct path to the files.',
                    'file-away' ),
                'tamper2'              => __(
                    'There was an error completing your request. The path to the directory has not been properly defined.',
                    'file-away'
                ),
                ...
                'no_upload_support'    => __(
                    'Your browser does not support the File Upload API. Please update.',
                    'file-away'
                ),
                'exceeds_size'         => sprintf( __( 'This file exceeds the %s max file size.', 'file-away' ),
                    'prettymax' ),
                ...
                'multi_type'           => __(
                    'You may not specify a script filetype prior to a non-script filetype.',
                    'file-away'
                ),
                'upload_failure'       => sprintf(
                    __( 'Sorry about that, but %s could not be uploaded.', 'file-away' ),
                    'filename'
                ),
