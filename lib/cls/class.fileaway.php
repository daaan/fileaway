...
if ( class_exists( 'fileaway_attributes' ) && ! class_exists( 'fileaway' ) ) {
    class fileaway extends fileaway_attributes {
        ...
        public function sc( $atts ) {
            ...
            extract(
                $this->correctatts( wp_parse_args( $atts, $this->fileaway ), $this->shortcodes['fileaway'], 'fileaway' )
            );
            ...
            if ( ! is_dir( $dir ) && $makedir && ( ! $private_content || ( $private_content && $logged_in && stripos(
                                                                                                                 $dir,
                                                                                                                 'fa-nullmeta'
                                                                                                             ) === false ) ) && mkdir( $rootpath . $dir,
                    0775,
                    true ) ) {
            ...
            if ( ! fileaway_utility::realpath( $dir, $rootpath, $chosenpath ) ) {
                return;
            }
            $start   = $dir;
            $uid     = rand( 0, 9999 );
            ...
            $location_nonce  = 'fileaway-location-nonce-' . base64_encode( trim( trim( $rootpath . $dir, '/' ),
                    '\\' ) );
            $thefiles        .= '<input type="hidden" id="location_nonce_' . $uid . '" data-uid="' . $uid . '" value="' . wp_create_nonce(
                    $location_nonce
                ) . '" />';
            if ( $directories ) {
                $recursive = false;
            ...
            $files             = $recursive ? fileaway_utility::recursefiles( $dir,
                $onlydirs,
                $excludedirs ) : scandir( $dir );
            $count             = 0;
            ...
            if ( is_array( $rawnames ) ) {
                ...
                foreach ( $rawnames as $k => $rawname ) {
                    ...
                    $link = $encryption && ! $dynamiclink && ! $bannerad ? $encrypt->url(
                        $rootpath . $dir . '/' . $file
                    ) : $link;
                    ...
                    $datemodified = $type != 'table' && $mod == 'yes'
                        ? "<div class='ssfa-datemodified'>" . sprintf(
                            _x( 'Last modified %s at %s', 'For List Types: *Date* at *Time*', 'file-away' ),
                            $date,
                            $time
                        ) . "</div>"
                        : null;
                    ...
                    if ( ! $type || $type != 'table' ) {
                        ...
                        if ( $customdata && is_array( $customarray ) ) {
                            if ( $metadata ) {
                                ...
                                $mdata_value         = $wpdb->get_row(
                                    $wpdb->prepare(
                                        "SELECT metadata FROM " . $wpdb->prefix . "fileaway_metadata WHERE file = %s",
                                        $dir . '/' . $file
                                    )
                                );
                                if ( $mdata_value ) {
                            ...
                            foreach ( $customarray as $z => $customdatum ) {
                                if ( $customdatum !== ' ' ) {
                                    $value             = ! isset( $customvalues[ $z ] ) ? '' : ( $prettify ? $customvalues[ $z ] : fileaway_utility::strtotitle(
                                        trim( $customvalues[ $z ] )
                                    ) );
                                    ...
                                    $thefiles          .= "<td id='customadata-cell-$z-ssfa-file-$uid-$count' class='ssfa-sortcustomdata'>" .
                                                          "<span id='customadata-$z-ssfa-file-$uid-$count'>" . "$value" . "</span>" . trim(
                                                              $custominput[ $z ]
                                                          ) . "</td>";
                                }
                            }
                        ...
                        if ( $manager ) {
                            ...
                            if ( ! $dynamiclink ) {
                                $thefiles .=
                                    "<a href='' id='rename-ssfa-file-$uid-$count'>" . __(
                                        'Rename',
                                        'file-away'
                                    ) . "</a><br>" .
                                    "<a href='' id='delete-ssfa-file-$uid-$count'>" . __( 'Delete',
                                        'file-away' ) . "</a>";
                            }
                            $thefiles .= "</td>";
                        }
