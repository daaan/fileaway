...
if (class_exists('fileaway_attributes') && !class_exists('fileup')) {
    class fileup extends fileaway_attributes
    {
        ...
        public function sc($atts)
        {
            ...
            $base = $base == 's2member-files' ? fileaway_utility::replacefirst(
                str_replace('\\', '/', WP_PLUGIN_DIR.'/s2member-files'),
                $chosenpath,
                ''
            ) : str_replace('\\', '/', $this->op['base'.$base]);
            $base = trim($base, '/');
            $base = trim($base, '/');
            ...
            if (!is_dir($dir) && $makedir && (!$private_content || ($private_content && $logged_in && stripos(
                            $dir,
                            'fa-nullmeta'
                        ) === false)) && mkdir($rootpath.$dir, 0775, true)) {
            ...
            $fixed = $start;
            $fixed = $fixedlocation ? ($problemchild ? fileaway_utility::replacefirst(
                $fixed,
                $install,
                ''
            ) : $fixed) : null;
            ...
            $path .= '<input type="hidden" id="location_nonce_'.$uid.'" data-uid="'.$uid.'" value="'.wp_create_nonce(
                    $location_nonce
                ).'" />';
            ...
            if ($filegroups) {
                $groups = preg_split(
                    '/(, |,)/',
                    strtolower(str_replace(' ', '', $filegroups)),
                    -1,
                    PREG_SPLIT_NO_EMPTY
                );
            ...
            $dropdown = $fixedlocation
                ...
                'style="display:block!important; margin-bottom:5px!important; text-align:left;">'.__(
            ...
            $form =
                $clearfix.
                '<div id="ssfa_fileup_container_'.$uid.'" class="ssfa_fileup_container '.$class.'" data-uid="'.$uid.'" data-mn="'.wp_create_nonce(
                    'fileaway-manager-nonce'
                ).'" style="'.$inlinestyle.' '.$fadeit.'">'.
