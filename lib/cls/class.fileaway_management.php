...
if ( ! class_exists( 'fileaway_management' ) ) {
    class fileaway_management {
        ...
        public function manager() {
            if ( empty( $_POST['act'] ) ) {
                return $this->response( $this->error( __( 'Invalid action', 'file-away' ) ) );
            }
            $action = sanitize_html_class( $_POST['act'] );
            if ( ! method_exists( $this, $action ) ) {
                return $this->response( $this->error( __( 'Invalid action', 'file-away' ) ) );
            }
            if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'fileaway-nonce' ) ) {
                return $this->response( $this->error( __( 'Nonce failed', 'file-away' ) ) );
            }
        ...
        private function error( $message = '' ) {
            if ( empty( $message ) ) {
                $message = __( 'An unknown error occurred.', 'file-away' );
            }
        ...
        private function response( $response = array() ) {
            if ( ! is_array( $response ) ) {
                $response = $this->error( __( 'Improperly formatted response', 'file-away' ) );
        ...
        private function flightbox() {
            ...
            if ( empty( $_POST['flightbox_nonce'] ) || ! wp_verify_nonce(
                    $_POST['flightbox_nonce'],
                    'fileaway-flightbox-nonce'
                ) ) {
                return $this->error( __( 'Flightbox nonce failed', 'file-away' ) );
            }
            ...
            if ( ( $s2 == 'true' || $s2 == 'skip' ) && $g == 'false' ) {
                ...
                $init   = fileaway_utility::replacefirst( $url,
            ...
            if ( $boxtype == 'image' ) {
                ...
                $download_link = $nolinks
                    ? null
                    : '<a href="' . fileaway_utility::urlesc(
                        $url
                    ) . '" class="ssfa-flightbox-download" ' . $linktype . $statstatus . '>' .
                ...
                (
                    'html'   =>
                        '<div id="ssfa-flightbox" class="' . sanitize_html_class(
                            $_POST['theme']
                        ) . '" style="display:inline-block; ' . $csize . ' left:' . $offset . 'px; top:' . $top . 'px; padding:0px!important;">' .
                        '<div id="ssfa-flightbox-inner" style="opacity:0; margin: 15px 15px 0!important;">' .
                        '<a href="' . fileaway_utility::urlesc(
                            $_POST['nexturl']
                        ) . '" onclick="' . $next . '"><img src="' . fileaway_utility::urlesc( $src ) . '" style="' . $isize . '"></a>' .
                        '</div>' .
                ...
                (
                    'html'    =>
                        '<div id="ssfa-flightbox" class="' . sanitize_html_class(
                            $_POST['theme']
                        ) . '" style="display:inline-block; ' . $csize . ' left:' . $offset . 'px; top:' . $top . 'px; padding:0!important;">' .
                ...
                (
                    'html'    =>
                        ...
                        '<div id="ssfa-flightbox-inner" style="opacity:0; margin: 15px 15px 0!important;">' .
                        '<iframe src="' . $src . '" frameborder=0 height="' . $height . '" width="' . $width . '" name="' . fileaway_utility::basename(
                            $src
        ...
        private function createdir() {
            ...
            if ( ! is_user_logged_in() ) {
                return $this->error( __( 'User error', 'file-away' ) );
            }
            if ( empty( $_POST['manager_nonce'] ) || ! wp_verify_nonce( $_POST['manager_nonce'],
                    'fileaway-manager-nonce' ) ) {
                return $this->error( __( 'Manager nonce failed', 'file-away' ) );
            }
            $parents = trim( str_replace( '..', '', fileaway_utility::stripslashes( $_POST['parents'] ) ), '/' );
            if ( ! fileaway_utility::realpath( $parents, $rootpath, $chosenpath ) ) {
                return $this->error( __( 'Path validation failed', 'file-away' ) );
            }
            if ( empty( $_POST['loc_nonce'] ) || ! fileaway_utility::verify_location_nonce(
                    $_POST['loc_nonce'],
                    $parents,
                    array( $rootpath, $chosenpath )
                ) ) {
                return $this->error( __( 'Location nonce failed', 'file-away' ) );
            }
            ...
            if ( is_dir( $final ) ) {
                return $this->error( __( 'That directory name already exists in this location.', 'file-away' ) );
            } else {
                ...
                if ( mkdir( $final, 0755, true ) ) {
                    if ( ! fileaway_utility::realpath( $parents . '/' . $newsub, $rootpath, $chosenpath ) ) {
                        ...
                        fileaway_utility::indexmulti( $rootpath . $parents . '/' . $newsub,
                            $rootpath . $parents . '/' );
                        if ( ! $first_exists ) {
                            $status  = "insert";
                            $message =
                                ...
                                "<a href=\"" . fileaway_utility::querystring(
                                    get_permalink( $page ),
                                    $querystring,
                                    array( $drawerid => $drawer )
                                ) .
                                ...
                                "<a class='ssfa-classic' href=\"" . fileaway_utility::querystring(
                                    get_permalink( $page ),
                                    $querystring,
                                    array( $drawerid => $drawer )
                                ) . "\">" .
                            ...
                            $message .=
                                ...
                                "<a href='' id='rename-ssfa-dir-$uid-$count'>" . __( 'Rename',
                                    'file-away' ) . "</a><br>" .
        ...
        private function renamedir() {
            ...
            $oldpath           = trim( str_replace( '..', '', fileaway_utility::stripslashes( $_POST['oldpath'] ) ),
                '/' );
            if ( ! fileaway_utility::realpath( $oldpath, $rootpath, $chosenpath ) ) {
                return $this->error( __( 'Path validation failed', 'file-away' ) );
            }
            ...
            if ( $rootpath . $check_path == $rootpath ) {
                return $this->error( __( 'Empty Path', 'file-away' ) );
            }
            if ( $rootpath . $check_path == WP_CONTENT_DIR ) {
                return $this->error( __( 'WPC Error', 'file-away' ) );
            }
            if ( strpos( $check_path, 'wp-admin' ) !== false ) {
                return $this->error( __( 'WPA Error', 'file-away' ) );
            }
            if ( strpos( $check_path, 'wp-includes' ) !== false ) {
                return $this->error( __( 'WPI Error', 'file-away' ) );
            }
            if ( is_file( $rootpath . $check_path . '/wp-config.php' ) ) {
                return $this->error( __( 'Install Location Error', 'file-away' ) );
            }
            $page        = (int) $_POST['pg'];
            $querystring = ltrim( $_POST['querystring'], '?' );
            $drawer      = str_replace( '/', '*', $newdata );
            $drawerid    = sanitize_html_class( $_POST['drawerid'] );
            $newurl      = fileaway_utility::querystring( get_permalink( $page ),
                $querystring,
                array( $drawerid => $drawer ) );
            if ( is_dir( $dst ) ) {
                return $this->error( __( 'That directory already exists.', 'file-away' ) );
            } elseif ( ! is_dir( $src ) ) {
                return $this->error( __( "The directory you're trying to rename could not be found.", 'file-away' ) );
            } else {
                if ( ! is_dir( $dst ) ) {
                    mkdir( $dst, 0755, true );
                }
                if ( ! fileaway_utility::realpath( $newpath, $rootpath, $chosenpath ) ) {
                                    ...
                                    if ( rename( $file, $filedest . '/' . $filename ) ) {
                                        ...
                                        if ( $table_exists ) {
                                            fileaway_utility::updatestats(
                                                ...
                                                fileaway_utility::replacefirst( $filedest . '/' . $filename,
                                                    $rootpath,
                                                    '' )
                                            );
                                        }
                    ...
                    if ( $fcount > 0 && ! $fscount ) {
                        return $this->error(
                            __(
                                'We tried to move the files into the newly-named directory but none of them would budge.',
                                'file-away'
                            )
                        );
                        ...
                        return $this->error(
                            __(
                                "We tried to move the files into the newly-named directory, but there were some stragglers, so we couldn't remove the old directory.",
                                'file-away'
                            )
                        );
                    } elseif ( ! is_dir( $src ) ) {
                        return array( 'url' => $newurl, 'newdata' => $newdata, 'newname' => $newname );
                    } else {
                        return $this->error( __( 'An unspecified error occurred.', 'file-away' ) );
                    }
                }
            }
        }
        ...
        private function deletedir() {
            ...
            if ( ! fileaway_utility::realpath( $path, $rootpath, $chosenpath ) ) {
                return $this->error( __( 'Path validation failed', 'file-away' ) );
            }
            if ( empty( $_POST['loc_nonce'] ) || ! fileaway_utility::verify_location_nonce(
                    $_POST['loc_nonce'],
            ...
            if ( is_file( $src . '/wp-config.php' ) ) {
                return $this->error( __( 'Install Location Error', 'file-away' ) );
            }
            if ( ! is_dir( $src ) ) {
                return $this->error( __( 'The directory marked for deletion could not be found.',
                        'file-away' ) . ' ' . $path );
            } else {
                $dirs = fileaway_utility::recursivedirs( $src );
                ...
                if ( $status === 'life' ) {
                    ...
                    if ( $fcount == 0 ) {
                        ...
                        $filemsg = null;
                        if ( $fcount >= 1 ) {
                            $plufiles = $fcount > 1 ? _x( 'files', 'plural', 'file-away' ) : _x(
                                'file',
                                'singular',
                                'file-away'
                            );
                        ...
                        $dirmsg = null;
                        if ( $dcount >= 1 ) {
                            $pludirs = $dcount > 1 ? _x( 'sub-directories', 'plural', 'file-away' ) : _x(
                                'sub-directory',
                                'singular',
                                'file-away'
                            );
                        ...
                        $message = sprintf(
                            _x(
                                'You are about to delete 1 directory%s from the server. ' .
                                'This action is permanent and cannot be undone. Are you sure you wish to proceed?',
                                'Do not put a space between *directory* and the %s variable',
                                'file-away'
                            ),
                            $dirmsg . $filemsg
                        );
                ...
                if ( $status === 'death' ) {
                    ...
                    if ( ( $pscount && $fscount ) || ( $pscount && ! $fcount ) ) {
                        ...
                        if ( $fcount >= 1 ) {
                            ...
                            $filemsg  = ' ' . __( 'and', 'file-away' ) . ' ' . $fscount . ' ' . __(
                                    'of',
                                    'file-away'
                                ) . ' ' . $fcount . ' ' . $plufiles;
                        } else {
                        ...
                        $message = sprintf(
                            _x(
                                '%d of 1 directory%s have been removed from the server.',
                                'Do not put a space between *directory* and the %s variable',
                                'file-away'
                            ),
                            $pscount,
                        ...
                        return $this->error(
                            __( 'Sorry, but there was an error attempting to remove this directory.', 'file-away' )
                        );
                    }
                }
            }
        ...
        private function rename() {
            ...
            $pp                = $problemchild ? fileaway_utility::replacefirst(
                fileaway_utility::stripslashes( $_POST['pp'] ),
                $install,
                ''
            ) : fileaway_utility::stripslashes( $_POST['pp'] );
            ...
            if ( ! fileaway_utility::realpath( $pp, $rootpath, $chosenpath ) ) {
                return $this->error( __( 'Path validation failed', 'file-away' ) );
            }
            $ext = sanitize_html_class( $_POST['ext'] );
            if ( strpos( strtolower( $ext ), 'php' ) !== false ) {
                return $this->error( __( 'Forbidden file type', 'file-away' ) );
            }
            if ( empty( $ext ) ) {
                return $this->error( __( 'Unknown file type', 'file-away' ) );
            }
            ...
            if ( $chosenpath . $pp == $rootpath ) {
                return $this->error( __( 'Empty Path', 'file-away' ) );
            }
            if ( $chosenpath . $pp == WP_CONTENT_DIR ) {
                return $this->error( __( 'WPC Error', 'file-away' ) );
            }
            if ( strpos( $chosenpath . $pp, 'wp-admin' ) !== false ) {
                return $this->error( __( 'WPA Error', 'file-away' ) );
            }
            if ( strpos( $chosenpath . $pp, 'wp-includes' ) !== false ) {
                return $this->error( __( 'WPI Error', 'file-away' ) );
            }
            if ( is_file( $chosenpath . $pp . '/wp-config.php' ) ) {
                return $this->error( __( 'Install Location Error', 'file-away' ) );
            }
            if ( empty( $_POST['loc_nonce'] ) || ! fileaway_utility::verify_location_nonce(
                    $_POST['loc_nonce'],
            ...
            if ( ! fileaway_utility::realpath(
                fileaway_utility::dirname( $pp . '/' . $oldname . '.' . $ext ),
                $rootpath,
                $chosenpath
            ) ) {
                return $this->error( __( 'Path validation failed', 'file-away' ) . ': 2' );
            }
            ...
            if ( ! $metadata ) {
                ...
                if ( is_array( $customdata ) ) {
                    ...
                    foreach ( $customdata as $datum ) {
                        ...
                        if ( fileaway_utility::stripslashes( $datum ) != '' ) {
                            $not_empty = true;
                        }
                    }
                }
                ...
                $newurl      = str_replace( $pp . '/' . $oldname . '.' . $ext,
                    "",
                    fileaway_utility::urlesc( $url, true ) );
        ...
        private function delete() {
            ...
            if ( in_array( $ext, array( 'php', 'htaccess', 'htpasswd' ) ) ) {
                return $this->error( __( 'Invalid File Type', 'file-away' ) );
            }
            if ( is_file( $chosenpath . $pp . '/wp-config.php' ) ) {
                return $this->error( __( 'Install Location Error', 'file-away' ) );
            }
            if ( is_file( $rootpath . $pp . '/wp-config.php' ) ) {
                return $this->error( __( 'Install Location Error', 'file-away' ) );
            }
            if ( strpos( $oldname, 'wp-admin' ) !== false ) {
                return $this->error( __( 'WPA Error', 'file-away' ) );
            }
            if ( strpos( $oldname, 'wp-config' ) !== false ) {
                return $this->error( __( 'WP Config Error', 'file-away' ) );
            }
            if ( strpos( $oldname, 'wp-includes' ) !== false ) {
                return $this->error( __( 'WPI Error', 'file-away' ) );
            }
            $oldfile = $rootpath . $pp . '/' . $oldname . '.' . $ext;
            if ( ! is_file( $oldfile ) ) {
                $oldfile = fileaway_utility::stripslashes( $oldfile );
            }
            if ( is_file( $oldfile ) && fileaway_utility::realpath(
                    fileaway_utility::dirname( $pp . '/' . $oldname . '.' . $ext ),
                    $rootpath,
                    $chosenpath
                ) ) {
                ...
                return $this->error( __( 'That file could not be found.', 'file-away' ) );
            }
        }

        private function bulkdownload() {
            extract( $this->pathoptions );
            if ( empty( $_POST['bulkdownload_nonce'] ) || ! wp_verify_nonce(
                    $_POST['bulkdownload_nonce'],
                    'fileaway-bulk-download-nonce'
                ) ) {
                return $this->error( __( 'Bulk Download nonce failed', 'file-away' ) );
            }
            $files    = $_POST['files'];
            $stats    = $_POST['stats'];
            $zipfiles = array();
            ...
            if ( is_array( $files ) ) {
                foreach ( $files as $file ) {
                    ...
                    if ( stripos( $file, 'wp-config' ) !== false ) {
                        continue;
                    }
                    if ( stripos( $file, 'wp-admin' ) !== false ) {
                        continue;
                    }
                    if ( stripos( $file, 'wp-includes' ) !== false ) {
                        continue;
                    }
                    if ( is_file( fileaway_utility::dirname( $rootpath . $file ) . '/wp-config.php' ) ) {
                        continue;
                    }
                    if ( empty( $_POST['loc_nonce'] ) || ! fileaway_utility::verify_location_nonce(
                            $_POST['loc_nonce'],
                    ...
                    if ( ! fileaway_utility::realpath( fileaway_utility::dirname( $file ), $rootpath, $chosenpath ) ) {
                        continue;
                    }
                    $file = $rootpath . $file;
                    if ( file_exists( $file ) ) {
            ...
            if ( count( $zipfiles ) ) {
                ...
                foreach ( $zipfiles as $k => $zipfile ) {
                    ...
                    if ( $numvals[ fileaway_utility::basename( $zipfile ) ] > 1 ) {
                        ...
                        $zip->renameName(
                            fileaway_utility::basename( $zipfile ),
                            $parts['filename'] . '_' . $k . '.' . $parts['extension']
                        );
                    }
                }
                $zip->close();
        ...
        private function bulkcopy() {
            ...
            foreach ( $from as $k => $fro ) {
                ...
                if ( ! fileaway_utility::realpath( fileaway_utility::dirname( $fro ), $rootpath, $chosenpath ) ) {
                    continue;
                }
                if ( strpos( $fro, '..' ) !== false ) {
                    continue;
                ...
                if ( stripos( $fro, '.php' ) !== false ) {
                    continue;
                }
                if ( stripos( $fro, 'wp-config' ) !== false ) {
                    continue;
                }
                if ( stripos( $fro, 'wp-admin' ) !== false ) {
                    continue;
                }
                if ( stripos( $fro, 'wp-includes' ) !== false ) {
                    continue;
                }
                if ( is_file( fileaway_utility::dirname( $chosenpath . $fro ) . '/wp-config.php' ) ) {
                    continue;
                }
                if ( empty( $_POST['loc_nonce'] ) || ! fileaway_utility::verify_location_nonce(
                        $_POST['loc_nonce'],
            ...
            if ( $success == 1 ) {
                return array(
                    'message' => sprintf(
                        __( 'One file was copied to %s and it no longer feels special.', 'file-away' ),
                        $destination
                    ),
                );
            }
            ...
            return array(
                'message' => sprintf(
                    __( '%d of %d files were successfully cloned and delivered in a black caravan to %s.',
                        'file-away' ),
                    $success,
                    $total,
                    $destination
        ...
        private function bulkmove() {
            ...
            if ( empty( $success ) ) {
                return $this->error(
                    __( 'There was a problem moving the files. Please consult your local ouija specialist.',
                        'file-away' )
                );
            }
            if ( $success == 1 ) {
                return array(
                    'message' => sprintf(
                        __( 'One lonesome file was forced to leave all it knew and move to %s.', 'file-away' ),
                        $destination
                    ),
                );
            }
            ...
            return array(
                'message' => sprintf(
                    __( '%d of %d files were magically transported to %s.', 'file-away' ),
                    $success,
                    $total,
                    $destination
                ),
        ...
        private function bulkdelete() {
            ...
            if ( empty( $success ) ) {
                return $this->error(
                    __(
                        'There was a problem deleting the files. Please try pressing your delete button emphatically and repeatedly.',
                        'file-away'
                    )
                );
            }
            if ( $success == 1 ) {
                return array(
                    'message' => __(
                        'A million fewer files in the world is a victory. One less file, a tragedy. Farewell, file. Au revoir. Auf Wiedersehen. Adieu.',
                        'file-away'
                    ),
                );
            }
            ...
            return array(
                'message' => sprintf(
                    __( '%d of %d files were sent plummeting to the nether regions of cyberspace.', 'file-away' ),
                    $success,
                    $total
                ),
            );
        ...
        private function upload() {
            ...
            if ( ! isset( $_POST ) || $_SERVER['REQUEST_METHOD'] != "POST" ) {
                return $this->error( __( 'Invalid context for upload', 'file-away' ) );
            }
            if ( empty( $_POST['upload_nonce'] ) || ! wp_verify_nonce( $_POST['upload_nonce'],
                    'fileaway-fileup-nonce' ) ) {
                return $this->error( __( 'Upload nonce failed', 'file-away' ) );
            }
            $file_name  = str_replace(
                '..',
                '',
                strip_tags( fileaway_utility::stripslashes( $_FILES['upload_file']['name'] ) )
            );
            ...
            if ( empty( $check_ext ) || empty( $check_name ) ) {
                return $this->error( __( 'Empty filename or extension', 'file-away' ) );
            }
            ...
            if ( $uploader ) {
                ...
                if ( $user ) {
                    ...
                    if ( preg_match( '/\[([^\]]+)\]/', $new_name ) ) {
                        ...
                        $new_name = fileaway_utility::replacelast(
                            $new_name,
                            '.' . $extension,
            ...
            if ( $file_size > $max_file_size ) {
                return $this->error( __( 'Max file size exceeded', 'file-away' ) );
                ...
                return $this->error( __( 'File Type Error', 'file-away' ) );
            } elseif ( stripos( $extension, 'php' ) !== false ) {
                return $this->error( __( 'File Type Error', 'file-away' ) );
            } elseif ( strpos( $dir, '..' ) !== false ) {
                return $this->error( __( 'Location Error', 'file-away' ) );
            }
            if ( ! is_dir( $dir ) ) {
                mkdir( $dir, 0755, true );
            }
        ...
        private function actionpath() {
            ...
            if ( $security ) {
                $pieces = explode( '/',
                    trim( trim( fileaway_utility::replacefirst( $pathparts, $nocrumbs, '' ), '/' ), '/' ) );
            } else {
            ...
            foreach ( $pieces as $k => $piece ) {
                ...
                $breadcrumbs .= '<a href="javascript:" data-target="' . trim(
                        $piecelink[ $k ],
                        '/'
                    ) . '" id="ssfa-' . $fileup . 'action-pathpart-' . $k . '">'
        ...
        private function deletecsv() {
            extract( $this->pathoptions );
            if ( empty( $_POST['values_nonce'] ) || ! wp_verify_nonce( $_POST['values_nonce'],
                    'fileaway-values-nonce' ) ) {
                return $this->error( __( 'Values nonce failed', 'file-away' ) );
            }
        ...
        private function makecsv() {
            ...
            if ( is_file( $rootpath . $dir . '/' . $filename ) ) {
                return $this->error( __( 'A file by that name already exists in this directory.', 'file-away' ) );
            }
            if ( strpos( $filename,
                    '/' ) !== false && ! is_dir( $rootpath . $dir . '/' . fileaway_utility::dirname( $filename ) ) ) {
            ...
            if ( is_file( $rootpath . $dir . '/' . $filename ) ) {
                ...
                $querystring = ltrim( $_POST['querystring'], '?' );
                $files       = $recursive ? fileaway_utility::recursefiles(
                    $fullpath,
                    array(),
                    array(),
                    '[cC][sS][vV]'
                ) : glob( $fullpath . '/*.[cC][sS][vV]' );
                foreach ( $files as $k => $file ) {
                    $files[ $k ] = str_replace( '\\', '/', $file );
                }
                $file_index = array_search( $rootpath . $dir . '/' . $filename, $files );
                $link       = fileaway_utility::querystring(
                    get_permalink( (int) $_POST['pg'] ),
                    $querystring,
                    array(
                        'fa_csv'   => base64_encode( fileaway_utility::basename( $filename ) ),
                        'fa_index' => $file_index,
                    )
                );
        ...
        private function values() {
            ...
            if ( ! is_file( $src ) ) {
                return $this->error( __( 'File not found', 'file-away' ) );
            }
            $csv          = new fileaway_csv();
            $csv->sort_by = 'id';
            $csv->auto( $src );
        ...
        private function createcol() {
            ...
            if ( ! in_array( (string) $_POST['col'], $csv->titles ) ) {
                return $this->error( __( 'Sorry about that, but your changes could not be saved.', 'file-away' ) );
            }
            foreach ( $rows as $k => $v ) {
        ...
        private function colrename() {
            ...
            foreach ( $rows as $k => $v ) {
                if ( isset( $rows[ $k ][ (string) $_POST['oldname'] ] ) ) {
                    fileaway_utility::recreatecol(
                        $rows[ $k ],
                        $_POST['colnum'],
                        0,
                        (string) $_POST['newname'],
                        $rows[ $k ][ $_POST['oldname'] ]
                    );
        ...
        private function coldelete() {
            ...
            $headers = $csv->titles;
            $rows    = $csv->data;
            if ( isset( $headers[ $_POST['colnum'] ] ) ) {
                unset( $headers[ $_POST['colnum'] ] );
            }
            $headers = array_values( $headers );
            foreach ( $rows as $k => $v ) {
                if ( isset( $rows[ $k ][ $_POST['col'] ] ) ) {
        ...
        private function backupcsv() {
            ...
            $newfile = str_replace(
                '.' . $bits['extension'],
                ' [' . date( 'Y-m-d H-i-s', current_time( 'timestamp' ) ) . '].' . $bits['extension'],
                $src
            );
            if ( $csv->save( $newfile ) ) {
                return array();
