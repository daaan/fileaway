    ...
    class fileaway_admin {
        ...
        public function __construct() {
            ...
            if ( ! is_subclass_of( $this, 'fileaway_admin' ) ) {
                ...
                foreach ( array( 'post.php', 'post-new.php' ) as $hook ) {
                    add_action( "admin_head-$hook", array( $this, 'modalvars' ) );
                }
                add_action( 'wp_ajax_fileaway_tinymce', array( $this, 'tinymce' ) );
                if ( $this->options['postidcolumn'] == 'enabled' ) {
                    new attachaway_column;
                }
            }
        }
        ...
        public function version() {
            ...
            if ( ! file_exists( $themedir . '/' . $template ) ) {
                copy( fileaway_dir . '/templates/' . $template, $themedir . '/' . $template );
            } elseif ( $version < '2.4' ) {
                copy( fileaway_dir . '/templates/' . $template, $themedir . '/' . $template );
            }
            if ( $version < '3.6' ) {
                wp_clear_scheduled_hook( 'ssfa_scheduled_cleanup' );
            }
            if ( $version === (string) fileaway_version ) {
                return;
            }
        ...
        public function tinymce() {
            if ( ! check_ajax_referer( '_nonce_fileaway_tinymce', 'security', false ) ) {
                echo 'Ajax Error';
            } else {
                new fileaway_modal;
            }
        ...
        public function save() {
            ...
            foreach ( $reloaders as $reloader ) {
                if ( $settings[ $reloader ] != $this->options[ $reloader ] ) {
                    $reload = true;
                }
                if ( $reload ) {
                    break;
        ...
        public function page() {
            ...
            foreach ( $this->sections as $slug => $section ) {
                ...
                if ( $slug == 'config' ) {
                    include fileaway_dir . '/lib/inc/inc.admin.config-instructions.php';
                }
                if ( $slug != 'tutorials' && $slug != 'about' ) {
                    $output .= $this->display( $slug, $this->defaults );
                } elseif ( $slug == 'tutorials' ) {
                    include fileaway_dir . '/lib/inc/inc.admin.tutorials.php';
                } elseif ( $slug == 'about' ) {
                    include fileaway_dir . '/lib/inc/inc.admin.about.php';
                }
                $output .= '</div>';
                $panelindex ++;
            }
                ...
                switch ( $type ) {
                    case 'text':
                        ...
                        $output .=
                            ...
                            'name="fileaway_options[' . $id . ']" placeholder="' . $holder . '" value="' . stripslashes(
                    ...
                    case 'rolescaps':
                        ...
                        $output .= '<div class="' . $class . '" id="fileaway-container-' . $id . '">';
                        $output .= '<input type="' . $input . '" id="' . $id . '" name="fileaway_options[' . $id . ']" value="' . esc_attr(
                                $this->options[ $id ]
                            ) . '" />';
                    ...
                    case 'users':
                        if ( $this->options['loadusers'] == 'true' ) {
                            ...
                            $output .= '<input type="text" class="regular-text fileaway-overridepassword ' . $class . '" ' .
                                       'id="' . $id . '" name="fileaway_options[' . $id . ']" value="' . esc_attr(
                                           $this->options[ $id ]
                                       ) . '" />';
                            $output .= $helplink . '</div>';
                        }
                    ...
                    case 'basefeed':
                        ...
                        if ( $title ) {
                            $output .= '<div class="fileaway-label"><label for="' . $id . '">' . $title . '</label>' .
        ...
        public function enqueue() {
            if ( isset( $_REQUEST['page'] ) && $_GET['page'] === 'file-away' ) {
                wp_enqueue_style(
                    'fileaway-admin-css',
                    fileaway_url . '/lib/css/admin.options.css',
                    array(),
                    fileaway_version
                );
                wp_enqueue_style( 'fileaway-chozed',
                    fileaway_url . '/lib/js/chosen/admin.chosen.css',
                    array(),
                    '1.1.0' );
                wp_enqueue_script( 'fileaway-chozed',
                    fileaway_url . '/lib/js/chosen/chosen.js',
                    array( 'jquery' ),
                    '1.1.0' );
                wp_enqueue_script( 'fileaway-footable',
                    fileaway_url . '/lib/js/footable.js',
                    array( 'jquery' ),
                    '2.0.1.2' );
                wp_enqueue_script( 'fileaway-filertify',
                    fileaway_url . '/lib/js/filertify.js',
                    array( 'jquery' ),
                    '0.3.11' );
                wp_enqueue_script(
                    'fileaway-clipboard',
                    fileaway_url . '/lib/js/clipboard/ZeroClipboard.js',
                    array( 'jquery' ),
                    '1.2.1'
                );
                wp_enqueue_script(
                    'fileaway-admin-js',
                    fileaway_url . '/lib/js/admin.options.js',
                    array( 'jquery' ),
                    fileaway_version
                );
                wp_localize_script( 'fileaway-admin-js', 'fileaway_admin_ajax', array(
                        'ajaxurl' => admin_url( 'admin-ajax.php' ),
                        'nonce'   => wp_create_nonce( 'fileaway-admin-nonce' ),
                    )
                );
                if ( $this->options['css_editor'] === 'syntax' ) {
                    wp_enqueue_style(
                        'fileaway-admin-codemirror',
                        fileaway_url . '/lib/js/codemirror/codemirror.css',
                        array(),
                        '4.4'
                    );
                    wp_enqueue_script(
                        'fileaway-admin-codemirror',
                        fileaway_url . '/lib/js/codemirror/codemirror.js',
                        array( 'jquery' ),
                        '4.4'
                    );
                    wp_enqueue_script(
                        'fileaway-admin-css',
                        fileaway_url . '/lib/js/codemirror/css.js',
                        array( 'jquery' ),
                        '4.4'
                    );
                }
                if ( $this->options['adminstyle'] == 'minimal' ) {
                    wp_enqueue_style(
                        'fileaway-admin-minimal',
                        fileaway_url . '/lib/css/admin.options-minimal.css',
                        array(),
                        fileaway_version
                    );
                }
            ...
            wp_enqueue_style( 'fileaway-modal-css',
                fileaway_url . '/lib/css/admin.modal.css',
                array(),
                fileaway_version );
