...
if ( class_exists( 'fileaway_attributes' ) && ! class_exists( 'fileaway_values' ) ) {
    class fileaway_values extends fileaway_attributes {
        ...
        public function sc( $atts ) {
            ...
            extract( $this->correct( wp_parse_args( $atts, $this->fileaway_values ),
                $this->shortcodes['fileaway_values'] ) );
            ...
            if ( $filename ) {
                ...
                if ( isset( $_REQUEST['fa_csv'] ) && isset( $_REQUEST['fa_index'] ) ) {
                    ...
                    $selected   = fileaway_utility::querystring(
                        get_permalink(),
                        $_SERVER['QUERY_STRING'],
                        array( 'fa_csv' => $selected, 'fa_index' => $file_index )
                    );
                }
                $dir   = rtrim( $dir, '/' );
                ...
                if ( $showoptions ) {
                    $options      = array();
                    $original_url = fileaway_utility::querystring(
                        get_permalink(),
                        $_SERVER['QUERY_STRING'],
                        array( 'fa_csv', 'fa_index' ),
                        true
                    );
                    if ( is_array( $files ) ) {
                        foreach ( $files as $key => $file ) {
                            if ( isset( $file_index ) && $file_index == $key ) {
                                $dir = $recursive ? str_replace( fileaway_utility::basename( $file ),
                                    '',
                                    $file ) : $dir;
                                $dir = rtrim( $dir, '/' );
                            }
                            ...
                            $link           = fileaway_utility::querystring(
                                ...
                                $_SERVER['QUERY_STRING'],
                                array(
                                    'fa_csv'   => base64_encode( fileaway_utility::basename( $file ) ),
                                    'fa_index' => $key,
                                )
                            );
                            $option_display = $recursive ? ( $install ? fileaway_utility::replacefirst(
                                $file,
                                $install,
                                ''
                            ) : $file ) : preg_replace( '/.csv$/i', '', fileaway_utility::basename( $file ) );
                            $option_display = $recursive ? str_replace(
                                '/',
                                ' &gt; ',
                                $option_display
                            ) : $option_display;
                            $options[]      = '<option value="' . $link . '" ' . $is_selected . '>' . trim(
                                    $option_display,
                                    '/'
                                ) . '</option>';
                        }
                    ...
                    $newcsv        = $editor
                        ? '<span id="ssfa-new-csv-' . $uid . '" style="font-size:18px; position:relative; top:4px; left:' . $newcsvspace . 'px; color:#AAA; cursor:pointer;" ' .
                          'class="ssfa-icon-file-4" aria-hidden="true" data-uid="' . $uid . '" data-pg="' . $GLOBALS['post']->ID . '" data-path="' . base64_encode(
                              $dir
                          ) . '" ' .
            ...
            if ( $filename && is_file( $rootpath . $dir . '/' . $filename ) ) {
                ...
                $thefiles .=
                    ...
                    "data-theme=\"$theme\" data-src=\"" . base64_encode(
                        $dir . '/' . $filename
                    ) . "\" data-filename=\"" . fileaway_utility::basename( $filename ) . "\" " .
