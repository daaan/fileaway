            ...
            foreach ( $this->shortcodes as $shortcode => $array ) {
                ...
                foreach ( $array['types'] as $type ) {
                    ...
                    foreach ( $array as $key => $a ) {
                        if ( $key == 'type' || $key == 'options' || $key == 'sections' || $key == 'option' || $key == 'types' ) {
                            continue;
                        }
                        if ( $numtypes > 1 && ! isset( $a[ $type ] ) ) {
                            continue;
                        }
                        $src      = $numtypes > 1 ? $a[ $type ] : $a;
                        $newline  = strpos(
                                        $a['class'],
                                        'fileaway-first-inline'
                                    ) !== false ? '<div style="display:block; visibility:hidden;"></div>' : null;
                        ...
                        if ( $a['element'] == 'text' ) {
                            ...
                            if ( isset( $ops ) && is_array( $ops ) ) {
                                foreach ( $ops as $value => $option ) {
                                    $fields[ $shortcode ][ $type ][ $section ][ $i ] .= '<option value="' . esc_attr(
                                            $value
