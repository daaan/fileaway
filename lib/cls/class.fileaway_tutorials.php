    ...
    class fileaway_tutorials {
        ...
        public function optioninfo() {
            ...
            $optioninfo['images']         = array(
                'heading' => 'Images',
                'info'    => "Optional: If left blank, the default behavior is to list image files along with all other files. You can alternatively choose to exclude all image types from your display, or to show only image types in your display. Image types are: " . implode(
                        ', ',
                        $get->imagetypes
                    ),
            );
            ...
            foreach ( $get->filegroups as $group => $discard ) {
                ...
                $filegroups .= '<span style="color:red;">' . $group . ':</span> [' . implode(
                        ', ',
                        $get->filegroups[ $group ][2]
                    ) . ']<br>';
            }
            ...
            foreach ( $array as $key => $a ) {
                ...
                if ( $hastypes ) {
                    ...
                    if ( $key != 'type' ) {
                        ...
                        foreach ( $array['type']['options'] as $k => $v ) {
                            $types[] = $k == '' ? $array['type']['default'] : $k;
                        }
