    ...
    class feedaway {
        ...
        public function intervals( $schedules ) {
            $schedules['fifteenminutes']   = array(
                'interval' => 900,
                'display'  => __( 'Every 15 Minutes', 'file-away' ),
            );
            $schedules['thirtyminutes']    = array(
                'interval' => 1800,
                'display'  => __( 'Every 30 Minutes', 'file-away' ),
            );
            $schedules['fortyfiveminutes'] = array(
                'interval' => 2700,
                'display'  => __( 'Every 45 Minutes', 'file-away' ),
            );
            $schedules['sixhours']         = array(
                'interval' => 21600,
                'display'  => __( 'Every 6 Hours', 'file-away' ),
            );
            $schedules['weekly']           = array(
                'interval' => 604800,
                'display'  => __( 'Once Weekly', 'file-away' ),
            );
            $schedules['fortnightly']      = array(
                'interval' => 1209600,
                'display'  => __( 'Every Two Weeks', 'file-away' ),
            );
        ...
        public function feeds() {
            ...
            $wp_excludes   = array(
                'wp-admin',
                'wp-includes',
                basename( WP_CONTENT_DIR ) . '/themes',
                basename( WP_CONTENT_DIR ) . '/upgrade',
            );
            $startswith    = array(
                'fa-feed-logo',
                '_thumb_',
                'fileaway-url-parser',
                'fileaway-banner-parser',
                'index.htm',
                'index.php',
                '.ht',
            );
            ...
            if ( isset( $this->options['feed_excluded_exts'] ) && ! empty( $this->options['feed_excluded_exts'] ) ) {
                $endswith = array_unique(
                    array_merge(
                        $endswith,
                        preg_split( '/(, |,)/', trim( $this->options['feed_excluded_exts'] ), - 1, PREG_SPLIT_NO_EMPTY )
                    )
                );
            }
            $excludestrings = array();
            if ( isset( $this->options['feed_excluded_files'] ) && ! empty( $this->options['feed_excluded_files'] ) ) {
                $excludestrings = preg_split(
                    '/(, |,)/',
                    trim( $this->options['feed_excluded_files'] ),
                    - 1,
                    PREG_SPLIT_NO_EMPTY
                );
            }
            if ( ! isset( $this->options['feeds'] ) || ! $this->options['feeds'] || trim(
                                                                                        $this->options['feeds']
                                                                                    ) == '' || trim( $this->options['feeds'] ) == '/' ) {
                return;
            }
            ...
            if ( $globallogo ) {
                $globallogo = fileaway_utility::urlesc(
            ...
            $feedlimit = isset( $this->options['feedlimit'] ) && is_numeric( $this->options['feedlimit'] ) ? round(
                $this->options['feedlimit'],
                0
            ) : false;
            $hardlinks = array();
            $map       = is_file( $storage . '/fa-directory-map.csv' ) ? new fileaway_csv(
                $storage . '/fa-directory-map.csv'
            ) : false;
            $now       = time();
            foreach ( $basedirs as $basedir ) {
                ...
                foreach ( $dirs as $k => $dir ) {
                    ...
                    if ( fileaway_utility::startswith(
                        fileaway_utility::replacefirst( $dir, $rootpath, '' ),
                        $excluded_dirs
                    ) ) {
                        if ( is_file( $feed ) ) {
                            unlink( $feed );
                    ...
                    if ( $this->options['recursivefeeds'] == 'true' ) {
                        ...
                        if ( is_array( $initdirs ) && count( $initdirs ) > 0 ) {
                            foreach ( $initdirs as $subdir ) {
                                if ( in_array( fileaway_utility::replacefirst( $subdir, $rootpath, '' ),
                                    $excluded_dirs ) ) {
                                    continue;
                                }
                                ...
                                $sublink    = $map ? $this->csvsearch(
                                    $map->data,
                                    $map->titles,
                                    trim( fileaway_utility::replacefirst( $subdir, $rootpath, '' ), '/' )
                                ) : false;
                                $sublink    = $sublink ? $sublink : fileaway_utility::replacefirst(
                                    $subfeed,
                                    $rootpath,
                                    rtrim( $this->options['baseurl'], '/' ) . '/'
                                );
                                $subPubDate = $this->options['feeddates'] == 'false' ? null : "<pubDate>" . date(
                                        $datestring,
                                        filemtime( $subdir )
                                    ) . "</pubDate>\n" .
                                                                                              $subxml[] =
                    ...
                    if ( is_array( $initfiles ) ) {
                        foreach ( $initfiles as $i => $file ) {
                            ...
                            if ( fileaway_utility::endswith( strtolower( fileaway_utility::basename( $file ) ),
                                $endswith ) ) {
                                unset( $initfiles[ $i ] );
                                continue;
                            }
                            ...
                            if ( $this->options['feedlinks'] != 'false' ) {
                                if ( in_array( $ext, array( 'mp3', 'wav', 'ogg' ) ) ) {
                                    ...
                                    $ext,
                                    array( 'avi', 'mpeg', 'mp4', 'ogv', 'mov', 'webm', 'flv', 'wmv', 'mkv' )
                                ) ) {
                                    $mime =
                                        $ext == 'avi' ? 'video/avi' :
                                            ( $ext == 'mp4' ? 'video/mp4' :
                                ...
                                if ( stripos( $file, 's2member-files/' ) ) {
                                    ...
                                    $fileurl = rtrim(
                                                   ...
                                                   '/'
                                               ) . '/?s2member_file_download=' . $getsub[1];
                                } else {
                                    $fileurl = fileaway_utility::urlesc(
                                        fileaway_utility::replacefirst(
                                            $file,
                                            $rootpath,
                                            rtrim( $this->options['baseurl'], '/' ) . '/'
                        ...
                        $stripped_url = str_ireplace( array( 'http:', 'https:', 'www.', 'ww2.', '//' ),
                            '',
                            rtrim( $this->options['baseurl'], '/' ) );
                        ...
                        $channellink  = $map ? $this->csvsearch(
                            $map->data,
                            $map->titles,
                            trim( fileaway_utility::replacefirst( $dir, $rootpath, '' ), '/' )
                        ) : $this->options['baseurl'];
                        $description  = $channellink ? $channellink : fileaway_utility::replacefirst(
                            $dir,
                            $rootpath,
                            rtrim( $this->options['baseurl'], '/' ) . '/'
                        );
                        ...
                        if ( $feedlogo ) {
                            $xml .=
                                "<image>\n" .
                                "<url>" . $feedlogo . "</url>\n" .
                                "<title>" . str_replace( array( 'http://', 'https://', 'www.' ),
                                    '',
                                    $this->options['baseurl'] ) . " > " . $directory . "</title>\n" .
                        ...
                        if ( isset( $files['datemodified'] ) && count( $files['datemodified'] ) > 0 ) {
                            foreach ( $files['datemodified'] as $k => $file ) {
                                ...
                                $thename         = preg_replace( '/(?<=\d)-(?=\D)/', ' ', "$thename" );
                                $thename         = preg_replace( '/(?<=\D)-(?=\d)/', ' ', "$thename" );
                                $showsize        = $this->options['feedsize'] == 'false' ? null : "Size: " . fileaway_utility::formatBytes(
                                        $files['filesize'][ $k ]
                                    ) . ".";
                                $filedescription = $customvalue ? $customvalue . ' (' . $files['extension'][ $k ] . " file. " . $showsize : $files['extension'][ $k ] . " file. " . $showsize;
                                $pubDate         = $this->options['feeddates'] == 'false' ? null : "<pubDate>" . date(
                                        $datestring,
                                        $file
                                    ) . "</pubDate>\n";
