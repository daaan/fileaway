...
if ( ! class_exists( 'fileaway_stats' ) ) {
    class fileaway_stats {
        ...
        public function __construct() {
            ...
            if ( $this->ops['stats'] == 'true' && isset( $this->ops['recordlimit'] ) && $this->ops['recordlimit'] && is_numeric(
                    $this->ops['recordlimit']
                ) ) {
        ...
        public function ajax() {
            ...
            if ( $type == 's2member' ) {
                ...
                $response = str_replace( array( '%3F', '%26' ),
                    array( '?', '&' ),
                    fileaway_utility::urlesc( $_POST['file'] ) );
            } elseif ( $type == 'encrypted' ) {
                list( $trash, $file ) = explode( "?", $_POST['file'] );
                parse_str( $file, $fileParsed );
                extract( $fileParsed );
                $crypt    = new fileaway_encrypted;
                $file     = fileaway_utility::urlesc(
                    fileaway_utility::replacefirst( $this->decrypt( $fileaway ), $rootpath, '' ),
                    true
                );
                $response = $this->ops['baseurl'] . '?fileaway_download=1&fileaway=' . $crypt->encrypt(
                        $rootpath . $file
                    ) . '&nonce=' . wp_create_nonce( 'fileaway-download' );
            } else {
                $crypt    = new fileaway_encrypted;
                $file     = fileaway_utility::urlesc(
                    fileaway_utility::replacefirst( $_POST['file'], rtrim( $this->ops['baseurl'], '/' ) . '/', '' ),
                    true
                );
                $response = $this->ops['baseurl'] . '?fileaway_download=1&fileaway=' . $crypt->encrypt(
                        $rootpath . $file
        ...
        public function imail( $data ) {
            if ( $this->ops['instant_stats'] != 'true' ) {
                return false;
            }
            $sender = $this->ops['instant_sender'] ? $this->ops['instant_sender'] : ( get_option(
                'admin_email'
            ) ? get_option( 'admin_email' ) : false );
            if ( ! $sender || ! $this->ops['instant_recipients'] ) {
                return false;
            }
            $original_timezone = date_default_timezone_get();
            fileaway_utility::timezone();
            $sendername = $this->ops['instant_sender_name'] ? $this->ops['instant_sender_name'] : stripslashes(
            ...
            $subject    = str_replace(
                ...
                array(
                    stripslashes( get_bloginfo( 'site_name' ) ),
                    fileaway_utility::basename( $data['file'] ),
                    $data['timestamp'],
                ),
                $subject
            );
        ...
        public function cmail() {
            global $wpdb;
            $offset = null;
            if ( $this->ops['compiled_stats'] == 'false' ) {
                return false;
            }
            if ( $this->ops['compiled_stats'] == 'daily' ) {
                $offset = '-1 day';
            } elseif ( $this->ops['compiled_stats'] == 'weekly' ) {
                $offset = '-1 week';
            } elseif ( $this->ops['compiled_stats'] == 'fortnightly' ) {
                $offset = '-2 weeks';
            } elseif ( $offset == null ) {
                return;
            }
            ...
            $sender = $this->ops['compiled_sender'] ? $this->ops['compiled_sender'] : ( get_option(
                'admin_email'
            ) ? get_option( 'admin_email' ) : false );
            if ( ! $sender || ! $this->ops['compiled_recipients'] ) {
                return false;
            }
            $sendername = $this->ops['compiled_sender_name'] ? $this->ops['compiled_sender_name'] : stripslashes(
                get_bloginfo( 'site_name' )
            );
            $headers[]  = 'From: ' . $sendername . ' <' . $sender . '>';
            ...
            $dates      = $this->ops['compiled_stats'] == 'daily'
                ...
                : date( $datestring, strtotime( date( 'Y-m-d' ) . ' ' . $offset ) ) . ' - ' . date(
                    $datestring,
            ...
            $records    = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM " . self::$db . " WHERE timestamp >= %s AND timestamp <= %s ORDER BY timestamp DESC",
                    $begin,
                    $end
                ),
                ARRAY_A
            ...
            if ( ! $records ) {
                $message = __( 'No downloads were recorded during this period. Bummer.', 'file-away' );
            } else {
                $message = $subject . "\n";
                $message .= "\n+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n\n";
        ...
        public function insert( $file, $notify = true ) {
            ...
            if ( $this->ops['instant_stats'] == 'true' && $notify && $this->imail( $data ) ) {
                $data['notified'] = 1;
            } elseif ( $this->ops['instant_stats'] == 'true' && ! $notify ) {
                $data['notified'] = 1;
            }
            date_default_timezone_set( $original_timezone );
        ...
        public function lifespanpurge() {
            ...
            date_default_timezone_set( $original_timezone );
            $records = $wpdb->get_results(
                $wpdb->prepare( "SELECT id FROM " . self::$db . " WHERE timestamp <= %s", $cutoff ),
                ARRAY_N
            );
            if ( ! $records ) {
                return;
        ...
        private function decrypt( $file ) {
            ...
            foreach ( str_split( fileaway_utility::urlesc( $file, true ) ) as $s ) {
