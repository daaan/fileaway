...
if ( ! class_exists( 'fileaway_utility' ) ) {
    class fileaway_utility {
        ...
        public static function startswith( $source, $prefix ) {
            if ( is_array( $prefix ) ) {
                foreach ( $prefix as $p ) {
                    if ( strncmp( $source, $p, strlen( $p ) ) == 0 ) {
                        return true;
                    }
                }
        ...
        public static function ini( $setting, $conversion = true, $null_message = false, $size = false ) {
            ...
            if ( ! $conversion && $result && $result != '' && $result != null ) {
                return $result;
            } elseif ( ! $conversion && ( ! $result || $result == '' || $result == null ) ) {
                return $null_message ? $null_message : '10M';
            } elseif ( $conversion ) {
        ...
        public static function currentrole() {
            ...
            $prettyrole   = isset( $wp_roles->role_names[ $role ] ) ? translate_user_role(
        ...
        public static function dynamicpaths( $dir, $playbackpath = false ) {
            ...
            $fa_username  = $logged_in ? ( $op['strictlogin'] === 'true' ? $current_user->user_login : strtolower(
                $current_user->user_login
            ) ) : 'fa-nulldirectory';
            $fa_firstlast = $logged_in ? strtolower(
                ...
                if ( is_array( $umetas[1] ) ) {
                    foreach ( $umetas[1] as $umeta ) {
                        ...
                        if ( ! $metavalue || $metavalue == '' ) {
                            $metavalue = 'fa-nullmeta';
                        } else {
                            $feedback['fa_metavalues'][] = $metavalue;
                        }
            ...
            if ( $playbackpath ) {
                ...
                if ( stripos( $playbackpath, 'fa-userrole' ) !== false ) {
                    $feedback['private_content'] = true;
                    $feedback['playbackpath']    = str_ireplace( 'fa-userrole',
                        $fa_userrole,
                        $feedback['playbackpath'] );
                }
                if ( stripos( $playbackpath, 'fa-username' ) !== false ) {
                    $feedback['private_content'] = true;
                    $feedback['playbackpath']    = str_ireplace( 'fa-username',
                        $fa_username,
                        $feedback['playbackpath'] );
                }
                if ( stripos( $playbackpath, 'fa-firstlast' ) !== false ) {
                    $feedback['private_content'] = true;
                    $feedback['playbackpath']    = str_ireplace( "fa-firstlast",
                        $fa_firstlast,
                        $feedback['playbackpath'] );
                }
                if ( stripos( $playbackpath, 'fa-usermeta(' ) !== false ) {
                    ...
                    if ( is_array( $umetas[1] ) ) {
                        foreach ( $umetas[1] as $umeta ) {
                            ...
                            $feedback['playbackpath'] = str_ireplace(
                                'fa-usermeta(' . $umeta . ')',
                                $metavalue,
                                $feedback['playbackpath']
                            );
            ...
            foreach ( $directories as $directory ) {
                if ( $excludedirs ) {
                    foreach ( $excludedirs as $exclude ) {
                        if ( self::endswith( "$directory", "$exclude" ) ) {
                            continue 2;
                        }
                    }
                }
        ...
        public static function index( $dir = false ) {
            ...
            if ( is_file( $dir . '/index.php' ) || is_file( $dir . '/index.html' ) || is_file( $dir . '/index.htm' ) ) {
                return false;
            }
        ...
        public static function feeds( $excluded = false ) {
            $op   = get_option( 'fileaway_options' );
            $bkup = $op;
            if ( ! isset( $op['excluded_feeds'] ) || ! is_array( $op['excluded_feeds'] ) ) {
                $op['excluded_feeds'] = array();
            }
            if ( ! isset( $op['basefeeds'] ) || ! is_array( $op['basefeeds'] ) ) {
                $op['basefeeds'] = array();
            }
            if ( $op != $bkup ) {
                update_option( 'fileaway_options', $op );
            }
        ...
        public static function updatestats( $where = false, $value = false, $replace = false ) {
            $i = 0;
            global $wpdb;
            if ( ! $where || ! $value || ! $replace ) {
                return;
            }
            $records = $wpdb->get_results(
                $wpdb->prepare( "SELECT id FROM " . $wpdb->prefix . "fileaway_downloads WHERE " . $where . " = %s",
                    $value )
            );
            if ( $records ) {
                foreach ( $records as $record ) {
                    if ( $wpdb->update(
                        $wpdb->prefix . "fileaway_downloads",
                        array( $where => $replace ),
                        array( 'id' => $record->id )
                    ) ) {
                        $i ++;
        ...
        public static function updatemetadata( $metadata = false, $oldfile = false, $newfile = false ) {
            ...
            if ( $metadata !== false ) {
                $data['metadata'] = is_array( $metadata ) ? serialize( $metadata ) : $metadata;
            }
            $row = $wpdb->get_row(
                $wpdb->prepare( "SELECT id FROM " . $wpdb->prefix . "fileaway_metadata WHERE file = %s", $oldfile )
            );
            if ( ! $row ) {
                $row = $wpdb->get_row(
                    $wpdb->prepare( "SELECT id FROM " . $wpdb->prefix . "fileaway_metadata WHERE file = %s", $newfile )
                );
            }
            if ( $row && $metadata !== '' ) {
                $success = $wpdb->update( $wpdb->prefix . "fileaway_metadata", $data, array( 'id' => $row->id ) );
            } elseif ( $row && $metadata === '' ) {
                $success = $wpdb->delete( $wpdb->prefix . "fileaway_metadata", array( 'id' => $row->id ) );
            } elseif ( ! $row && $metadata !== false && ! empty( $metadata ) ) {
                $success = $wpdb->insert( $wpdb->prefix . "fileaway_metadata", $data );
            } else {
                $success = true;
            }
        ...
        public static function realpath( $path, $base1, $base2 ) {
            ...
            if ( trim( $check1, '/' ) == trim( str_replace( '\\', '/', ABSPATH ), '/' ) || trim( $check2, '/' ) == trim(
                    str_replace( '\\', '/', ABSPATH ),
                    '/'
                ) ) {
                return false;
            ...
            if ( strpos( $base1 . $path, '..' ) !== false || strpos( $base2 . $path, '..' ) !== false ) {
                return false;
            }
            $maybeSym = false;
            if ( ( strpos( $check1, $base1 ) !== 0 && strpos( $check2, $base2 ) !== 0 ) ) {
                $maybeSym = true;
            }
            if ( $check1 != $base1 . $path && $check2 != $base2 . $path ) {
                $maybeSym = true;
            }
            if ( ! $maybeSym ) {
                return true;
            }
            if ( ! fileaway_definitions::symlinks() ) {
                return false;
            }
            $isSym    = false;
            $basename = '';
        ...
        public static function basename( $path_file ) {
            return fileaway_definitions::$pathinfo ? self::pathinfo( $path_file, PATHINFO_BASENAME ) : basename(
                $path_file
            );
        }

        public static function dirname( $path_file ) {
            return fileaway_definitions::$pathinfo ? self::pathinfo( $path_file,
                PATHINFO_DIRNAME ) : dirname( $path_file );
        }
