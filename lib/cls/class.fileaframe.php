    ...
    class fileaframe extends fileaway_attributes
    {
        ...
        public function sc($atts)
        {
            ...
            if ($devices) {
                ...
                if ($devices === 'desktop' && $get->is_mobile) {
                    return;
                }
            }
            ...
            if (!$name) {
                return
                    sprintf(
                        __(
                            'Please assign your fileaframe shortcode a unique name, using %s, and assign the same name to its corresponding %s shortcode, using %s',
                            'file-away'
                        ),
                        '[fileaframe name="myuniquename"]',
                        '[fileaway]',
                        '[fileaway name="myuniquename"]'
                    );
            } elseif (!$source) {
