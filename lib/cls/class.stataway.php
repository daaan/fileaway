...
if ( class_exists( 'fileaway_attributes' ) && ! class_exists( 'stataway' ) ) {
    class stataway extends fileaway_attributes {
        ...
        public function sc( $atts ) {
            ...
            extract(
                $this->correctatts( wp_parse_args( $atts, $this->stataway ), $this->shortcodes['stataway'], 'stataway' )
            );
        ...
        public function userstats( $atts ) {
            ...
            extract( $this->correct( wp_parse_args( $atts, $this->stataway_user ),
                $this->shortcodes['stataway_user'] ) );
            global $wpdb;
            ...
            $records = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT file, timestamp FROM " . fileaway_stats::$db . " WHERE uid = %d AND timestamp >= %s AND timestamp <= %s ORDER BY timestamp DESC",
                    $userid,
                    $begin,
                    $end
                ),
            ...
            foreach ( $records as $i => $record ) {
                ...
                if ( $timestamp == 'yes' ) {
                    $items[ $i ] .= ' <span style="display:block;" class="' . $class . 'timestamp">' .
                                    date( $datestring . ' ' . get_option( 'time_format' ),
                                        strtotime( $record['timestamp'] ) ) . '</span>';
                }
                $items[ $i ] .= '</li>';
            }
