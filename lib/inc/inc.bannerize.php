...
if ( isset( $this->op['banner_directory'] ) ) {
    $bannerpath = rtrim( $this->op['banner_directory'], '/' );
} else {
    return;
}
if ( ! is_dir( $rootpath . $bannerpath ) || ! is_file( $rootpath . $bannerpath . '/' . $bannerfile ) || ! is_readable(
        $rootpath . $bannerpath . '/' . $bannerfile
    ) ) {
    return;
}
