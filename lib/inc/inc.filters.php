...
while ( ! $excluded ) {
    ...
    foreach ( $get->nevershows as $nevershow ) {
        if ( strripos( $file, $nevershow ) !== false ) {
            $excluded = true;
        }
    }
    if ( $excluded ) {
    ...
    $excluded = ! $manager && fileaway_utility::startswith( fileaway_utility::basename( $file ),
        '_thumb_' ) ? true : false;
    if ( $excluded ) {
        break;
    }
    $excluded = ! $manager && fileaway_utility::startswith(
        fileaway_utility::basename( $file ),
        'fa-feed-logo'
    ) ? true : false;
    if ( $excluded ) {
        break;
    ...
    if ( $images && ! $included ) {
        $is_image = 0;
        if ( in_array( strtolower( $extension ), $get->imagetypes ) ) {
            $is_image = 1;
        }
    }
    ...
    if ( $onlyaudio && ! $included ) {
        $is_audio = 0;
        if ( in_array( strtolower( $extension ), $get->filegroups['audio'][2] ) ) {
            $is_audio = 1;
        }
