...
$records = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT file FROM " . fileaway_stats::$db . " WHERE timestamp >= %s AND timestamp <= %s ORDER BY timestamp DESC",
        $begin,
        $end
    ),
    ARRAY_A
...
foreach ( $rawnames as $k => $rawname ) {
    ...
    if ( $s2mem && strpos( $dir, 's2member-files' ) !== false ) {
        ...
        $link   = $s2mem && strpos(
                                $dir,
                                's2member-files'
                            ) !== false ? $url . '/?s2member_file_download=' . $sub . $file . $s2skip : $link;
    }
