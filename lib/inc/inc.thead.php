...
if ( $type === 'table' ) {
    ...
    if ( $directories ) {
        $filename = $drawerlabel ? $drawerlabel : ( $filenamelabel ? $filenamelabel : _x(
            'Drawer/File',
            'File and Directory Name Column',
            'file-away'
        ) );
    } else {
        $filename = $filenamelabel ? $filenamelabel : _x( 'File&nbsp;Name', 'File Name Column', 'file-away' );
    }
    ...
    $typesorter         = $playback ? "data-sort-ignore='true'" : "title=\"" . _x(
            'Click to Sort',
            'Column Sort Message',
            'file-away'
        ) . "\" $typesort";
    ...
    $data_atts          = $manager ? 'data-uid="' . $uid . '" data-pg="' . $GLOBALS['post']->ID . '" data-drw="' . $drawericon . '" data-cls="' . $theme . '" data-metadata="' . $mdata . '" ' .
                                     'data-basename="' . $basename . '" data-start="' . $start . '" data-dir="' . trim(
                                         "$dir",
                                         '/'
                                     ) . '" data-base="' . $basebase . '" data-basedir="' . $basedir . '"' .
    ...
    $bulkdownload_nonce = $bulkdownload || $manager ? 'data-bd="' . wp_create_nonce(
    ...
    $thefiles           .= "<script type='text/javascript'>jQuery(function(){ jQuery('.footable').footable();});</script>" .
                           ...
                           "<th class='ssfa-sortname' title=\"" . _x(
                               'Click to Sort',
                               'Column Sort Message',
                               'file-away'
                           ) . "\"" . $filenamesort . ">$filename$path</th>";
    $cells              = array();
    if ( $mod !== 'no' ) {
        $cells[] = 1;
    }
    ...
    if ( $customdata ) {
        ...
        foreach ( $customarray as $customdatum ) {
            if ( $customdatum !== ' ' ) {
                ...
                $thefiles    .= "<th class='ssfa-sortcustomdata' title=\"" . _x(
                        'Click to Sort',
                        'Column Sort Message',
                        'file-away'
                    ) . "\"" . $custom_sort . ">$customdatum</th>";
            }
        }
    }
    $thefiles .= $mod !== 'no'
        ? "<th class='ssfa-sortdate' data-type='numeric' title=\"" . _x(
            'Click to Sort',
            'Column Sort Message',
            'file-away'
        ) . "\"" . $modsort . ">" . $datelabel . "</th>"
        : null;
    $thefiles .= $size !== 'no'
        ? "<th class='ssfa-sortsize' data-type='numeric' title=\"" . _x(
            'Click to Sort',
            'Column Sort Message',
            'file-away'
        ) . "\"" . $sizesort . ">" .
        ...
        : null;
    if ( $manager ) {
        $thefiles .= "<th style='width:90px!important;' class='ssfa-manager' data-sort-ignore='true'>" . _x(
                'Manage',
                'Manager Column',
                'file-away'
            ) . "</th>";
