        ...
        foreach ( $parsers as $parser ) {
            ...
            if ( file_exists( $parsepath . '/' . $parsefile ) && is_readable( $parsepath . '/' . $parsefile ) ) {
                ...
                foreach ( $dynamiclinks as $dl ) {
                    if ( isset( $dl['URL'] ) && ( preg_match( '/[a-z]/i', $dl['URL'] ) || preg_match( '/\d/',
                                $dl['URL'] ) ) ) {
                        $finalname   = isset( $dl['FILENAME'] ) && ( preg_match( '/[a-z]/i',
                                $dl['FILENAME'] ) || preg_match(
                                                                         '/\d/',
                                                                         $dl['FILENAME']
                                                                     ) )
                            ? $dl['FILENAME']
                            : str_replace(
                                '.',
                                '',
                                fileaway_utility::basename( $dl['URL'] )
                            );
                        $youtube     = preg_match(
