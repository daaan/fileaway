...
if ( $directories ) {
    ...
    if ( $manager && $dirman ) {
        $thefiles .=
            ...
            '<input id="input-ssfa-create-dir-' . $uid . '" type="text" placeholder="' . __(
                'Name Your Sub-Directory',
                'file-away'
            ) . '" " value="" ' .
        ...
        foreach ( $cells as $cell ) {
            $icell ++;
            if ( $icell < $ccell ) {
                $thefiles .= "<td class='$theme' data-value='# # # #'> &nbsp; </td>";
            } else {
                $thefiles .= "<td id='manager-ssfa-create-dir-$uid' class='$theme' data-value='# # # #'> &nbsp; </td>";
            }
        }
    }
    ...
    if ( count( $checksubdirs ) > 0 ) {
        ...
        foreach ( glob( $dir . '/*', GLOB_ONLYDIR ) as $k => $folder ) {
            ...
            if ( $color && $accent ) {
                $dir_colors = " ssfa-$color accent-$accent";
            }
            if ( ( $color ) && ! ( $iconcolor ) ) {
            ...
            if ( $excludedirs ) {
                foreach ( $excludedirs as $exclude ) {
                    if ( strripos( $folder, str_replace( '\\', '/', $exclude ) ) !== false ) {
                        continue 2;
                    }
                }
            }
            if ( ! $direxcluded ) {
                ...
                if ( $getrss ) {
                    $inilocation = is_file(
                        $rootpath . $basebase . '/' . trim( $dlink, '/' ) . '/_fa.feed.id.ini'
                    ) ? $rootpath . $basebase . '/' . trim( $dlink, '/' ) . '/_fa.feed.id.ini' : false;
                    if ( $inilocation ) {
                        $subini      = parse_ini_file( $inilocation );
                        $subfeedid   = $subini['id'];
                        $subfeedfile = $rootpath . trim(
                                str_replace( '\\', '/', $this->op['feeds'] ),
                                '/'
                            ) . '/_feed_' . $subfeedid . '.xml';
                        if ( is_file( $subfeedfile ) ) {
                            $subfeedurl = fileaway_utility::replacefirst(
                                $subfeedfile,
                                $rootpath,
                                rtrim( str_replace( '\\', '/', $this->op['baseurl'] ), '/' ) . '/'
                            );
