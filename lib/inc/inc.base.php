...
$s2mem = fileaway_definitions::$s2member && ( $base == 's2member-files' || stripos(
                                                                               $this->op[ 'base' . $base ],
                                                                               'plugins/s2member-files'
                                                                           ) !== false ) ? true : false;
$base  = $s2mem ? str_replace(
    '\\',
    '/',
    fileaway_utility::replacefirst( WP_PLUGIN_DIR . '/s2member-files', $chosenpath, '' )
