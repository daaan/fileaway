...
$thefiles .= $crumbies;
if ( $rsslink != null ) {
    $thefiles .= '<span style="float:left;">' . $rsslink . '</span>';
}
if ( $type === 'table' && $search !== 'no' ) {
    $thefiles .=
        ...
        "<input id='filter-$uid' class='ssfa-searchfield' placeholder='" . __(
            'SEARCH',
            'file-away'
        ) . "' value='' name='search' id='search' type='text' />" .
        "</div>";
}
$thefiles .= '</div>';