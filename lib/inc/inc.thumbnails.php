...
if ( $thumbnails ) {
    $getthumb = in_array(
        $extension,
        array( 'jpg', 'jpeg', 'gif', 'png', 'pdf', 'flv', 'mp4', 'm4v', 'webm', 'ogv', 'tube', 'vmeo' )
    ) ? true : false;
}
...
if ( $getthumb ) {
    if ( in_array( $extension, array( 'flv', 'mp4', 'm4v', 'webm', 'ogv', 'tube', 'vmeo' ) ) ) {
        ...
        if ( ! is_file( $thumbpath . '.jpg' ) && ! is_file( $thumbpath . '.png' ) && ! in_array( $extension,
                array( 'tube', 'vmeo' )
            ) ) {
            $getthumb = false;
        } else {
            ...
            if ( $extension == 'tube' ) {
                if ( is_file( $thumbpath . $vidthumbext ) ) {
                    $thumblink = fileaway_utility::urlesc( str_replace( $rootpath,
                            $url . '/',
                            $thumbpath ) . $vidthumbext );
                } else {
                ...
                $thumblink = str_replace(
                    $tempfile,
                    '_thumb_vid_' . fileaway_utility::replacelast( $tempfile, '.' . $oext, $vidthumbext ),
                    $links[ $k ]
                );
            }
        }
        ...
        if ( $extension == 'pdf' && function_exists( 'exec' ) ) {
            ...
            if ( ! is_file( $thumbpath ) ) {
                ...
                if ( $thumbsize == 'large' ) {
                    exec(
                        "convert -define jpeg:size=180x180 \"{$pdfpath}[0]\" -colorspace RGB -thumbnail 180x180 -gravity center -crop 180x180+0+0 +repage $thumbpath"
                    );
                } elseif ( $thumbsize == 'medium' ) {
                    exec(
                        "convert -define jpeg:size=120x120 \"{$pdfpath}[0]\" -colorspace RGB -thumbnail 120x120 -gravity center -crop 120x120+0+0 +repage $thumbpath"
                    );
                } else {
                    exec(
                        "convert -define jpeg:size=60x60 \"{$pdfpath}[0]\" -colorspace RGB -thumbnail 60x60 -gravity center -crop 60x60+0+0 +repage $thumbpath"
                    );
                }
            }
            $thumblink = is_file( $thumbpath ) ? str_replace(
                $tempfile,
                '_thumb_' . $thumbfix . fileaway_utility::replacelast( $tempfile, '.pdf', '.jpg' ),
                $links[ $k ]
            ) : false;
        } else {
            $tempfile = fileaway_utility::urlesc( $file );
            if ( ! is_file( $rootpath . $dir . '/_thumb_' . $thumbfix . $file ) ) {
                fileaway_utility::createthumb(
                    $rootpath . $dir . '/' . $file,
                    $rootpath . $dir . '/_thumb_' . $thumbfix . $file,
                    $extension,
                    $thumbwidth,
                    $thumbheight
                );
            }
            $thumblink = is_file( $rootpath . $dir . '/_thumb_' . $thumbfix . $file ) ? str_replace(
                $tempfile,
                '_thumb_' . $thumbfix . $tempfile,
                $links[ $k ]
            ) : false;
        }
    }
