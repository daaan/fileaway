...
$thefiles    .= "<script type='text/javascript'>jQuery(function(){ jQuery('.footable').footable();});</script>" .
                ...
                "<thead><tr>" .
                "<th class='ssfa-sorttimestamp $theme-first-column' $clicktosort data-sort-initial='descending' data-type='numeric'>" . __(
                    'Timestamp',
                    'file-away'
                ) . "</th>" .
                "<th class='ssfa-sortfile' $clicktosort>" . ucfirst( __( 'file', 'file-away' ) ) . "</th>" .
                "<th class='ssfa-sortuid' $clicktosort data-type='numeric' style='width:120px;'>" . str_replace(
                    ' ',
                    '&nbsp;',
                    __( 'User ID', 'file-away' )
                ) . "</th>";
$thefiles    .= $username !== 'no' ? "<th class='ssfa-sortusername' $clicktosort>" . __(
        'Username',
        'file-away'
    ) . "</th>" : null;
$thefiles    .= $email !== 'no' ? "<th class='ssfa-sortemail' $clicktosort>" . __( 'Email',
        'file-away' ) . "</th>" : null;
$thefiles    .= $ip !== 'no' ? "<th class='ssfa-sortip' data-type='numeric' $clicktosort>" . __(
        'IP Address',
        'file-away'
    ) . "</th>" : null;
$thefiles    .= $agent == 'yes' ? "<th class='ssfa-sortagent' $clicktosort>" . __( 'User Agent',
        'file-away' ) . "</th>" : null;
$thefiles    .= "</tr></thead><tfoot><tr><td colspan='100'>$pagearea</td></tr></tfoot><tbody>";
$records     = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT * FROM " . fileaway_stats::$db . " WHERE timestamp >= %s AND timestamp <= %s ORDER BY timestamp DESC",
        $begin,
        $end
    ),
    ARRAY_A
...
if ( $records && count( $records ) ) {
    ...
    foreach ( $records as $record ) {
        ...
        $count ++;
        $filecol  = $filecolumn == 'file' ? fileaway_utility::basename( $record['file'] ) : str_replace(
            '/',
            ' > ',
            $record['file']
        );
    ...
    foreach ( $rows as $k => $row ) {
        foreach ( $csv->titles as $header ) {
            $cols[ $header ] = $rows[ $k ][ $header ];
        }
        $rows[ $k ] = $cols;
    }
    $csv->data = $rows;
    $filename  = 'Download Stats (' . date( 'Y-m-d', strtotime( $begin ) ) . ' - ' . date( 'Y-m-d',
            strtotime( $end ) ) . ').csv';
    $csv->save( fileaway_dir . '/temp/' . $filename );
    $csvlink = '<a class="stataway-csv" href="' . fileaway_url . '/temp/' . $filename . '" download>Download CSV</a>';
    if ( is_file( fileaway_dir . '/temp/' . $filename ) ) {
        $thefiles .= $csvlink;
    }
} else {
    $thefiles .= '</tbody></table>';
